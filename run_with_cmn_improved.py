...
def main():
    ...
    class Args:
        def __init__(self):
            ...
            self.d_model = 512
            self.d_inner = 2048
            self.dropout = 0.2  # 增加dropout防止过拟合
            self.pe_maxlen = 5000
            self.d_word_vec = 512
            self.n_layers_dec = 6
            self.tgt_emb_prj_weight_sharing = 1
            self.label_smoothing = 0.1
            self.epochs = 30  # 增加训练轮数
            self.shuffle = 1
            self.batch_size = 16  # 减小batch size
            self.batch_frames = 0
            self.maxlen_in = 50
            self.maxlen_out = 25
            self.num_workers = 2
    ...
    # 开始训练
    train_net_improved(args)
    
    print("训练完成!")

def train_net_improved(args):
    torch.manual_seed(7)
    np.random.seed(7)
    ...
    if checkpoint is None:
        ...
        optimizer = TransformerOptimizer(
            torch.optim.AdamW(model.parameters(), betas=(0.9, 0.98), eps=1e-09, weight_decay=1e-4))
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        train_loss = train_improved(train_loader=train_loader,
                                   model=model,
                                   optimizer=optimizer,
                                   epoch=epoch,
                                   logger=logger,
        ...
        valid_loss = valid_improved(valid_loader=valid_loader,
                                   model=model,
                                   logger=logger)
...
def train_improved(train_loader, model, optimizer, epoch, logger, writer):
    model.train()
...
def valid_improved(valid_loader, model, logger):
    model.eval()
